...
CCoordSysGraphView::CCoordSysGraphView() noexcept
	...
	coord2D_{ CoordSys2D::Space::ClientSpace{{-1, 1}, {-1, -1}, {1, -1}},
		...
		std::optional{std::array{
			std::optional{ typename CoordSys2D::TickAttributes{0.1F, 3, 6, tickPen_.get(), tickBrush_.get()} },
		...
		std::optional{std::array{
			...
			std::optional{ typename CoordSys2D::LabelAttributes{0.1F, coordsys::LabelFormatter<CoordSys2D::ClientUnit>{coordLabelFormat}, CoordSys2D::ScreenVector{8, -15}, (TA_TOP | TA_NOUPDATECP | TA_CENTER), nullptr, 0, 0.1F} }
		}}
	}
{
...
{
	const CString inputText = edit.GetEditText();
...
void CCoordSysGraphView::setBoundingValueToEditField(CMFCRibbonEdit& edit) const noexcept {
	...
	{
		const auto formattedValue = formatter(coord2D_.getAxisEnd(CoordSys2D::Axis::Abscissa, CoordSys2D::Axis::PointIndex::Start));
		edit.SetEditText(CString{ CA2TEX<64>{formattedValue.data()} });

		break;
	}
	case ID_EDIT_COORD_YMIN: 
	{
		const auto formattedValue = formatter(coord2D_.getAxisEnd(CoordSys2D::Axis::Ordinate, CoordSys2D::Axis::PointIndex::Start));
		edit.SetEditText(CString{ CA2TEX<64>{formattedValue.data()} });
	...
	{
		const auto formattedValue = formatter(coord2D_.getAxisEnd(CoordSys2D::Axis::Abscissa, CoordSys2D::Axis::PointIndex::End));
		edit.SetEditText(CString{ CA2TEX<64>{formattedValue.data()} });
	...
	{
		const auto formattedValue = formatter(coord2D_.getAxisEnd(CoordSys2D::Axis::Ordinate, CoordSys2D::Axis::PointIndex::End));
		edit.SetEditText(CString{ CA2TEX<64>{formattedValue.data()} });
...
void CCoordSysGraphView::drawScene(HDC paintDC) noexcept {
	...
	coord2D_.ellipse(rasteriser, 5.0F, 2.2F, 1, 0.75F);
}
